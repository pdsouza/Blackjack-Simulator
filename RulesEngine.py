	...
	def update(self, standing):
		for player in standing:
			for i in xrange(0,len(player.hands)):
				# Blackjack
				if (player.val[i] == 21 and len(player.hands[i]) == 2) and (self.dealer.val[0] != 21 or len(self.dealer.hands[0]) != 2):
					player.cash += int((5.0/2.0)*player.bet)
					player.bet = int((3.0/2.0)*player.bet)
