...
class Bot(Player):
	...
	def __init__(self, name, cash=3000):
		...
		# "K"elly Criterion optimal betting rates
	...
	def play(self, dealer, hand=0):
		...
		'''
		if self.val > 21: return -1
		if len(self.hands[hand]) == 2 and self.val in (10,11):
			self.doubled_down = True
			self.cash -= self.bet 
			self.bet *= 2
			#print 'DOUBLED DOWN: ' + str(self.bet)
			return 1
		if self.val < 17 and not self.doubled_down and not (self.val,dealer.hands[0][1][1]) in self.strategy: return 1
		return 0'''
	...
	def update(self, players): # Bot can see both dealer cards?
		dealer = players[-1]
		player = players[:-1]
	...
	def make_bet(self):
		...
		if self.tcount < 0: # Don't play if house has advantage
			self.bet = 0
		#print 'C"A"SH: ' + str(self.cash)
		self.cash -= self.bet
		#print 'RUNNING COUNT: ' + str(self.rcount)
		print 'TRUE COUNT: ' + str(self.tcount)
		#print 'R"A"TE: ' + str(self.rate)
